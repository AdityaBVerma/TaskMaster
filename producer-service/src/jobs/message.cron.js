            ...
            for(const task of tasks){
                for(const user of task.assignedto){
                    await messageQueue.add(
                        ...
                        {jobId: `${task._id}-${user.email}`, attempts: 3, backoff:{ type: "exponential", delay: 5000}, removeOnComplete: true} //NOTE: jobId is so that bullmq dosent re add the same job if existing in the queue
                    )
                    console.log(`Produced Message job ${task.id}-${task.name} for ${user.email}`)
